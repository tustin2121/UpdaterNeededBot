...
const MAX_PAST_XMLS = 50;
const sock = io();
let paused = false;
let pastXmls = [];

sock.on('ledger', (i, xml)=>{
	pastXmls.unshift(parseXml(xml));
	if (pastXmls.length >= MAX_PAST_XMLS) { pastXmls.pop(); }
	drawPastBuffer();
...
$('#pastBuffer').css({ 'grid-template-columns': '1fr '.repeat(MAX_PAST_XMLS) });
$('#playpause').prop('disabled', true);
drawPastBuffer();

function pauseUpdate(p) {
	paused = !!p;
	$('#playpause').toggleClass('paused', paused);
	if (!paused) {
		pastXmls.forEach(x=>x.sel = false);
		let { xml } = pastXmls[0];
		displayXml(xml);
		drawPastBuffer();
	}
...
function drawPastBuffer() {
	let $pb = $('#pastBuffer').empty();
	let sel = false;
	for (let i = 0; i < MAX_PAST_XMLS; i++) {
		let xml = pastXmls[i];
		if (!xml) continue;
		let $b = $('<buffer>').appendTo($pb);
		let $l = $('<level>').appendTo($b);
		$l.height(xml.lvl);
		if (xml.lvl > 60) $l.addClass('overload');
		if (xml.sel) $b.addClass('selected');
		if (xml.out) $b.addClass('output');
		if (xml.battle) $b.addClass('inBattle');
		$b.on('click', ()=>{
			pauseUpdate(true);
			pastXmls.forEach(x=>x.sel = false);
			xml.sel = true;
	...
	if (!sel) $pb.children(':first').addClass('selected');
}

function parseXml(xml) {
	let $xml = $(xml);
	let lvl = Number.parseInt($xml.attr('lvl'), 10);
	let api = Number.parseInt($xml.children('api').text(), 10);
	let out = !!$xml.children('update').text();
	let battle = !!$xml.find('ledgeritem[name=BattleContext]').length;
	return { xml, lvl, api, out, battle, sel:false };
}
